...
export default function inspectObject ( obj, objName ) {
  const result = [];
  ...
  (function inspect ( o ) {
    ...
    Reflect.ownKeys( o )
          ...
          .forEach( prop => {
            ...
            if (!desc) {
              return add( prop, '', {}, '<unreadable>')
            }
            if (desc.get) {
              add( prop, 'get ', desc, `f get ${prop.toString()}() : ${ safeStringify( obj[ prop ] ) }` );
            }
            if (desc.set) {
    ...
    if (proto === null) {
      ...
      add( '__proto__', '', {}, (proto.name ? `${proto.name} (constructor)` : proto.constructor.name) );
      level++;
      inspect( proto );
    }
  })( obj );
