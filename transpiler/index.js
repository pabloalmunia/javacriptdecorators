            ...
            if (decorator.kind.substring (0, 5) === 'init-') {
              ...
              if (!staticInitializersCreated) {
                ...
                insertBeforeNext (parent, nextElement, [{
                  ...
                  'expression' : {
                    'type'      : 'CallExpression',
                    'callee'    : I (staticInitializersName + '.forEach'),
                    'arguments' : [
                      {
                        ...
                        'body'       : {
                          'type'      : 'CallExpression',
                          'callee'    : I ('initialize.call'),
                          'arguments' : [I (className), I (className)]
                        }
                      }
                    ]
                  }
        ...
        if (decorator.kind === 'init-class') {
          ...
          if (!classInitializersCreated) {
            ...
            insertBeforeNext (parent, nextElement, [{
              ...
              'expression' : {
                'type'      : 'CallExpression',
                'callee'    : I (classInitializersName + '.forEach'),
                'arguments' : [
                  {
                    'type'       : 'ArrowFunctionExpression',
                    'expression' : true,
              ...
              if (!memberInitializersCreated) {
                ...
                if (o.static) {
                  ...
                  addIntoConstructor (klass, [{
                    ...
                    'expression' : {
                      'type'      : 'CallExpression',
                      'callee'    : I (memberInitializersName + '.forEach'),
                      'arguments' : [
                        {
                          'type'   : 'ArrowFunctionExpression',
                          'params' : [I ('initialize')],
    ...
    isStatic ?
      {
        ...
        'expression' : {
          'type'      : 'CallExpression',
          'callee'    : I (globalInitializerName + '.set.call'),
          'arguments' : [
            I (className),
            {
              'type'      : 'CallExpression',
...
function accessorGenerator ({className, initializeName, propertyName, decoratorName, isStatic, isSymbol}) {
  ...
  return [
      ...
      'declarations' : [
        {
          ...
          'init' : {
            'type'      : 'CallExpression',
            'callee'    : I ('Object.getOwnPropertyDescriptor'),
            'arguments' : [
              I (isStatic ? className : className + '.prototype'),
              isSymbol ?
                I (propertyName) :
    ...
    },
    {
      ...
      'expression' : {
        'type'      : 'CallExpression',
        'callee'    : I ('Object.defineProperty'),
        'arguments' : [
          (isStatic) ?
            I (className) :
...
function privateFieldGenerator ({kind, className, elementName, decoratorName, variableName, symbolGetName, symbolSetName, isStatic}) {
  return [
    ...
    isStatic ? {
        ...
        'expression' : {
          'type'      : 'CallExpression',
          'callee'    : I (className + '.' + symbolSetName, true),
          'arguments' : [
            {
              'type'      : 'CallExpression',
              'callee'    : I (variableName),
              'arguments' : [
                {
                  'type'      : 'CallExpression',
                  'callee'    : I (className + '.' + symbolGetName, true),
                  'arguments' : []
                }
              ]
            }
